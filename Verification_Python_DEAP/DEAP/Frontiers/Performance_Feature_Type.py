...
f1_scores = np.zeros((rhythmNum, featureTypeNum, subNum))
acc_scores = np.zeros((rhythmNum, featureTypeNum, subNum))

for f_no in range(0,featureTypeNum):
    for rhy_no in range(0,rhythmNum):
        xfile = sio.loadmat('.//SubX_FeatureType//subX_feature_type_'+str(f_no+1)+'_'+rhythms[rhy_no]+'_DEAP.mat')
        subX_all = xfile['f']
        ## scale data
        ...
        for subNo in range(0, subNum):
            ...
            f1_scores[rhy_no, f_no, subNo] = metrics.f1_score(testY, predict_testY)
            acc_scores[rhy_no, f_no, subNo] = metrics.accuracy_score(testY, predict_testY)
            print('featureType: ', f_no, ' rhythm: ', rhythms[rhy_no], ' sub: ', subNo, ' performance: ', acc_scores[rhy_no, f_no, subNo])


# save results
sio.savemat('acc_scores_subcross_featuretype.mat', {'results': acc_scores})
sio.savemat('f1_scores_subcross_featuretype.mat', {'results': f1_scores})
