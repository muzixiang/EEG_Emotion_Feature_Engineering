...
probas_labels = np.zeros((subNum*trialNum,3,3))
...
for subNo in range(0, subNum):
    ...
    # method 1
    sel_criteria1 = SelectKBest(chi2, k=num_ks[0]).fit(trainX, trainY)
    ...
    probas_labels[subNo * trialNum:(subNo + 1) * trialNum, 0, 0:2] = predict_probas
    probas_labels[subNo * trialNum:(subNo + 1) * trialNum, 0, 2] = testY

    # method 2
    sel_criteria2 = SelectKBest(mutual_info_classif, k=num_ks[1]).fit(trainX, trainY)
    ...
    print predict_probas
    probas_labels[subNo * trialNum:(subNo + 1) * trialNum, 1, 0:2] = predict_probas
    probas_labels[subNo * trialNum:(subNo + 1) * trialNum, 1, 2] = testY

    # method 3
    sel_criteria3 = SelectKBest(f_classif, k=num_ks[2]).fit(trainX, trainY)
    ...
    print predict_probas
    probas_labels[subNo * trialNum:(subNo + 1) * trialNum, 2, 0:2] = predict_probas
    probas_labels[subNo * trialNum:(subNo + 1) * trialNum, 2, 2] = testY

#save results
if emodim==0:
    #save results
    sio.savemat('valence_predict_probability_filter_revision.mat', {'probas_labels': probas_labels})
else:
    sio.savemat('arousal_predict_probability_filter_revision.mat', {'probas_labels': probas_labels})
