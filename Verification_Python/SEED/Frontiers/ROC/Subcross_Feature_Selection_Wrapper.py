...
probas_labels = np.zeros((subTrialNum*subNum,3))
...
for subNo in range(0, subNum):
    ...
    sel_indx = sel_indx[0]
    trainX1 = trainX[:, sel_indx]
    testX1 = testX[:, sel_indx]

    #svm
    clf1 = svm.SVC(kernel='linear', probability=True)
    clf1.fit(trainX1, trainY)
    predict_probas = clf1.predict_proba(testX1)
    probas_labels[subNo * subTrialNum:(subNo + 1) * subTrialNum, 0:2] = predict_probas
    probas_labels[subNo * subTrialNum:(subNo + 1) * subTrialNum, 2] = testY
    print('current sub performance:', predict_probas)

#save results

sio.savemat('predict_probability_RFE.mat', {'probas_labels': probas_labels})
